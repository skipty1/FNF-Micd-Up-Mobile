...
class MainVariables
{
    ...
    {
        File.saveContent((Main.path + 'config.json'), Json.stringify(_variables));
    }

    public static function Load():Void
    {

        if (!FileSystem.exists(Main.path + 'config.json'))
        {
            _variables = {
                resolution: 1,
                fullscreen: false,
        ...
        {
            var data:String = File.getContent((Main.path + 'config.json'));
